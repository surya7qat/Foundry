...
const CustomerTab: React.FC = () => {
    ...
    const [searchQuery, setSearchQuery] = useState(() => {
        return sessionStorage.getItem('customer_search_query') || '';
    });
    ...
    const fetchCustomers = async (pageNumber: number, searchVal?: string) => {
        const term = searchVal !== undefined ? searchVal : searchQuery;
        try {
            const res = await api.get(`/api/inventory/customers/?page=${pageNumber}&page_size=10&search=${encodeURIComponent(term)}`);
            if (res.data.results) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (editingId) {
                await api.put(`/api/inventory/customers/${editingId}/`, payload);
                showToast('Customer updated successfully', 'success');
            } else {
                await api.post('/api/inventory/customers/', payload);
                showToast('Customer added successfully', 'success');
                setPage(1);
            }
            cancelEdit();
            fetchCustomers(editingId ? page : 1);
        } catch (err: any) {
            if (err.response?.data) {
                setErrors(err.response.data);
                showToast('Unable to save customer. Please check the fields.', 'error');
            } else {
    ...
    const handleSearch = () => {
        sessionStorage.setItem('customer_search_query', searchQuery);
        setPage(1);
    ...
    const handleClearSearch = () => {
        setSearchQuery('');
        sessionStorage.removeItem('customer_search_query');
        setPage(1);
        fetchCustomers(1, '');
    };

    const formatAddress = (cust: Customer) => {
        const parts = [
            cust.address_line1,
            cust.address_line2,
            cust.area,
    ...
    return (
        <div className="inventory-master-page">
            <div className="page-header" style={{ marginBottom: '2rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1rem' }}>
                <div>
                    <h1 style={{ fontFamily: "'Orbitron', monospace", fontSize: '2.2rem', margin: 0, background: 'var(--gradient-molten)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>Customer Master</h1>
                    <p style={{ color: '#ccc', fontSize: '1.1rem', marginTop: '0.5rem' }}>Manage all client profiles here.</p>
                </div>
                {viewMode === 'list' && (
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem', minWidth: 0 }}>
                                    ...
                                    <tbody>
                                        {customers.length === 0 ? (
                                            ...
                                            <tr key={cust.id} className={(editingId === cust.id ? 'editing-row ' : '') + (!cust.is_active ? 'inactive-row' : '')}>
                                                    ...
                                                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                                                        <span 
                                                            style={{ 
                                                                ...
                                                                height: '8px', 
                                                                borderRadius: '50%', 
                                                                background: cust.is_active ? '#22c55e' : '#ef4444',
                                                                boxShadow: cust.is_active ? '0 0 8px #22c55e' : '0 0 8px #ef4444',
                                                                display: 'inline-block',
                                                                flexShrink: 0
                                                            }} 
                                                            title={cust.is_active ? 'Active' : 'Inactive'}
                                                        />
                                                        <span style={{ fontWeight: '600', color: '#fff' }}>{cust.name}</span>
                                                    </div>
                                                </td>
                                                <td style={{ fontWeight: '600', fontFamily: 'monospace' }}>{cust.code}</td>
                                                <td>
                                                    {cust.gst_number ? (
                                                ...
                                                <td style={{ color: '#aaa', fontSize: '0.9rem', maxWidth: '200px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }} title={formatAddress(cust)}>
                                                    {formatAddress(cust)}
                                                </td>
                                                <td style={{ fontFamily: 'monospace' }}>{cust.pan || '-'}</td>
                                                <td>
                                                    <div style={{ display: 'flex', gap: '8px', justifyContent: 'center', alignItems: 'center' }}>
                                                        <button className="action-icon-btn edit-btn" onClick={() => handleEdit(cust)} title="Edit Customer">
                                                            <Edit2 size={16} />
                                                        </button>
                                                        {sessionStorage.getItem('is_superuser') === 'true' && (
                                                            <button className="action-icon-btn history-btn" onClick={() => setAuditLogData({ created_by: cust.created_by, created_at: cust.created_at, updated_by: cust.updated_by, updated_at: cust.updated_at })} title="Audit Log" style={{ background: 'rgba(59,130,246,0.15)', color: '#3b82f6', border: '1px solid rgba(59,130,246,0.3)' }}>
                                                                <History size={16} />
                                                            </button>
                                                        )}
                                                    </div>
                        ...
                        <div className="data-table-wrapper mobile-only-view">
                            <div className="mobile-card-list" style={{ padding: '1rem' }}>
                                {customers.length === 0 ? (
                                    <div style={{ textAlign: 'center', padding: '2rem', color: '#888' }}>No customers found.</div>
                                ) : customers.map(cust => (
                                    <div className="mobile-card" key={cust.id}>
                                        <div className="mobile-card-header" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                                            <h3>{cust.name}</h3>
                                            <span style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '0.85rem', fontWeight: 600, color: cust.is_active ? '#4ade80' : '#f87171' }}>
                                                <span style={{ width: '8px', height: '8px', borderRadius: '50%', background: cust.is_active ? '#22c55e' : '#ef4444', boxShadow: cust.is_active ? '0 0 8px #22c55e' : '0 0 8px #ef4444' }}></span>
                                                {cust.is_active ? 'Active' : 'Inactive'}
                                            </span>
                                        </div>
                                        <div className="mobile-card-body">
                                            <p><strong>Customer ID</strong> <span className="monospace-text" style={{ color: 'var(--color-molten-yellow)' }}>{cust.customer_id}</span></p>
                                            <p><strong>Code</strong> <span className="monospace-text">{cust.code}</span></p>
                                            <p><strong>GST Number</strong> <span className="monospace-text">{cust.gst_number || '-'}</span></p>
                                            <p><strong>Address</strong> <span style={{ textAlign: 'right', fontSize: '0.9rem' }}>{formatAddress(cust)}</span></p>
                                            <p><strong>PAN</strong> <span className="monospace-text">{cust.pan || '-'}</span></p>
                                        </div>
                                        <div className="mobile-card-actions">
                                            ...
                                            {sessionStorage.getItem('is_superuser') === 'true' && (
                                                <button className="action-icon-btn history-btn" onClick={() => setAuditLogData({ created_by: cust.created_by, created_at: cust.created_at, updated_by: cust.updated_by, updated_at: cust.updated_at })} title="Audit Log" style={{ background: 'rgba(59,130,246,0.15)', color: '#3b82f6', border: '1px solid rgba(59,130,246,0.3)', display: 'flex', alignItems: 'center', gap: '4px', padding: '6px 12px', borderRadius: '4px', fontSize: '0.85rem' }}>
                                                    <History size={16} /> Log
                                                </button>
                                            )}
                                        </div>
                    ...
                    <div className="form-panel" style={{ width: '100%', maxWidth: '600px' }}>
                        ...
                        <form onSubmit={handleSubmit}>
                            ...
                            <div className="input-group">
                                <label>Customer ID <span className="required-asterisk">*</span></label>
                                <input className="glass-input" required maxLength={15} pattern="^[A-Za-z0-9_\-]+$" title="Alphanumeric, dashes, and underscores only. Max 15 chars." value={formData.customer_id} onChange={e => setFormData({...formData, customer_id: e.target.value.toUpperCase()})} />
                                {errors.customer_id && <span className="error-text">{errors.customer_id}</span>}
                            </div>

                            {/* Customer Name */}
                            <div className="input-group">
                                <label>Customer Name <span className="required-asterisk">*</span></label>
                            ...
                            {/* Customer Code */}
                            <div className="input-group">
                                <label>Customer Code <span className="required-asterisk">*</span></label>
                            ...
                            <div className="input-group" style={{ flexDirection: 'row', alignItems: 'center', marginTop: '1rem', background: 'rgba(255,255,255,0.02)', padding: '10px', borderRadius: '6px' }}>
                                <input type="checkbox" id="cust-active" checked={formData.is_active} onChange={e => setFormData({...formData, is_active: e.target.checked})} style={{ width: '18px', height: '18px', accentColor: 'var(--color-accent)', cursor: 'pointer' }} />
                                <label htmlFor="cust-active" style={{ margin: 0, color: 'white', cursor: 'pointer', fontWeight: 500, fontSize: '0.95rem' }}>Active Record</label>
                            </div>
