...
const MaterialStockLogTab: React.FC = () => {
    ...
    const [searchQuery, setSearchQuery] = useState(() => {
        return sessionStorage.getItem('material_stock_log_search_query') || '';
    });
    ...
    const fetchLogs = async (pageNum = 1, searchVal = searchQuery) => {
        setLoading(true);
        try {
            const res = await api.get(`/api/inventory/material-stock-correction-log/`, {
                params: {
                    page: pageNum,
                    search: searchVal
                }
            ...
            console.error(err);
            showToast('Failed to fetch raw material stock correction logs.', 'error');
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleSearch = () => {
        setPage(1);
        sessionStorage.setItem('material_stock_log_search_query', searchQuery);
    ...
    const handleClearSearch = () => {
        setSearchQuery('');
        setPage(1);
        sessionStorage.removeItem('material_stock_log_search_query');
        fetchLogs(1, '');
    };
    ...
    return (
        <div className="tab-container" style={{ position: 'relative' }}>
            <div className="tab-header">
                <h2>Raw Material Stock Correction Log</h2>
            </div>
            ...
            <div className="search-bar-container" style={{ display: 'flex', gap: '10px', marginBottom: '1.5rem', flexWrap: 'wrap' }}>
                <div className="search-input-wrapper" style={{ position: 'relative', flex: 1, minWidth: '200px' }}>
                    ...
                    <input 
                        type="text" 
                        placeholder="Search by Material, Batch No, Reason..." 
                        value={searchQuery}
            ...
            <div className="data-table-wrapper desktop-only-view" style={{ position: 'relative' }}>
                ...
                <div className="data-table-container" ref={tableContainerRef}>
                    <table className="glass-table">
                        ...
                        <tbody>
                            {loading ? (
                                <tr><td colSpan={6} style={{ textAlign: 'center', padding: '2rem' }}>Loading correction logs...</td></tr>
                            ) : logs.length === 0 ? (
                                <tr><td colSpan={6} style={{ textAlign: 'center', padding: '2rem' }}>No correction logs found.</td></tr>
                            ) : logs.map(l => (
                                <tr key={l.id}>
                                    ...
                                    <td className="wrap-text">
                                        <div style={{ fontWeight: '600', wordBreak: 'break-all' }}>{l.raw_material_name}</div>
                                        <div style={{ fontSize: '0.8rem', color: 'var(--color-molten-yellow)', fontFamily: 'monospace', wordBreak: 'break-all' }}>Batch: {l.batch_no || '-'}</div>
                                    </td>
                                    <td style={{ fontFamily: 'monospace' }} className="wrap-text">{l.quantity.toFixed(2)} {l.material_unit}</td>
                                    <td style={{ fontFamily: 'monospace', fontWeight: '700', color: 'var(--color-glow-green)' }} className="wrap-text">{l.corrected_quantity.toFixed(2)} {l.material_unit}</td>
                    ...
                    {loading ? (
                        ...
                        <div className="mobile-card" key={l.id} style={{ borderLeft: '3px solid var(--color-molten-yellow)' }}>
                            <div className="mobile-card-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                <h3 style={{ wordBreak: 'break-all' }}>{l.raw_material_name}</h3>
                            ...
                            <div className="mobile-card-body">
                                ...
                                <div style={{ display: 'flex', justifyContent: 'space-between', background: 'rgba(255,255,255,0.02)', padding: '8px', borderRadius: '4px', margin: '8px 0', border: '1px solid rgba(255,255,255,0.05)' }}>
                                    <div>
                                        ...
                                        <div style={{ fontFamily: 'monospace', fontWeight: 'bold' }}>{l.quantity.toFixed(2)} {l.material_unit}</div>
                                    </div>
                                    <div style={{ textAlign: 'right' }}>
                                        <div style={{ fontSize: '0.75rem', color: '#aaa' }}>Corrected Qty</div>
                                        <div style={{ fontFamily: 'monospace', fontWeight: 'bold', color: 'var(--color-glow-green)' }}>{l.corrected_quantity.toFixed(2)} {l.material_unit}</div>
                                    </div>
                                </div>
...
export default MaterialStockLogTab;
